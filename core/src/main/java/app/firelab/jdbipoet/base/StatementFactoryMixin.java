...
public interface StatementFactoryMixin extends StatementFactory {
  @Override
  default  <E extends LazyExpression<?>> InsertStatementBuilderStep1 insertInto(LazyExpression<?> table) {
    return null;
  }

  @Override
  default SelectStatement select(String raw, List<Object> bindings) {
    return Sql.statements().select(raw, bindings);
  }
